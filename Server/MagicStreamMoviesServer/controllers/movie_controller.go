...
var validate=validator.New()//help to validate the data as mentioned in `` of Server/models/movie_model.go

func GetMovies(client *mongo.Client)gin.HandlerFunc{

	return func(c *gin.Context){ //response to the client (r,w) and this is not just w over here
		//gin.Context encapsulates both the request (r) and the response writer (w), plus it holds route parameters, query strings, and helper methods to send JSON, HTML, etc.
		...
		cursor, err:= movieCollection.Find(ctx, bson.M{}) //bson.M is map[string]interface{}
...
func GetMovie(client *mongo.Client) gin.HandlerFunc{ //by ImdbID
	return func(c *gin.Context){
		...
		if movieID==""{
			c.JSON(http.StatusBadRequest,gin.H{"error":"movie id is required, stupid, the imdb one"})
			return
		}
		...
		err:=movieCollection.FindOne(ctx, bson.M{"imdb_id":movieID}).Decode(&movie); if err!=nil{
...
func AddMovie(client *mongo.Client) gin.HandlerFunc{
	return func(c *gin.Context){
		...
		if err:=validate.Struct(movie); err!=nil{
			c.JSON(http.StatusBadRequest, gin.H{"error":"the validation failed lol"})
			return
		}
		var movieCollection *mongo.Collection=database.OpenCollection("movies",client)


		result,err:=movieCollection.InsertOne(ctx,movie); if err!=nil{
			c.JSON(http.StatusInternalServerError,gin.H{"error":"insertion of movie addition to db"})
		}
...
func AdminReviewUpdate(client *mongo.Client)gin.HandlerFunc{
	return func(c *gin.Context){
		...
		role,err:=utils.GetRoleFromContext(c);if err!=nil{
...
func GetReviewRanking(admin_review string, client *mongo.Client, c *gin.Context)(string,int, error){
	rankings,err:=GetRankings(client, c);if err!=nil{
		return "",0,err
	...
	if err!=nil{
		log.Println("Warning: the env file is not loaded")
	}

	// we are not using this, bcoz my open ai api key is not normal, it is actually from azure open ai space
	...
	llm, err := getAzureLLMClient()//instead made a helper function to simulate the same thing
	if err!=nil{
		return "",0, err
	}
...
// getAzureLLMClient configures the OpenAI client to transparently act as an Azure proxy
func getAzureLLMClient() (*openai.LLM, error) {
	apiKey := os.Getenv("AZURE_OPENAI_API_KEY")
	baseURL := os.Getenv("AZURE_OPENAI_BASE_URL")
	deployment := os.Getenv("AZURE_OPENAI_DEPLOYMENT_NAME")

	if apiKey == "" || baseURL == "" || deployment == "" {
		return nil, errors.New("missing critical Azure OpenAI environment variables in .env")
	}
...
func GetRecommendedMovies(client *mongo.Client) gin.HandlerFunc{
	return func(c *gin.Context){
		userId,err:=utils.GetUserIdFromContext(c);if err!=nil{
...
func GetUsersFavouriteGenres(userId string, client *mongo.Client, c *gin.Context)([]string, error){
	...
	for _,item:=range favGenresArray{
		if genreMap,ok:= item.(bson.D);ok{
			for _,eleme:=range genreMap{
				if eleme.Key=="genre_name"{
					if name,ok:=eleme.Value.(string);ok{
