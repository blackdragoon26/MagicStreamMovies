...
func ValidateToken(tokenString string)(*SignedDetails, error){
	...
	if _,ok:=token.Method.(*jwt.SigningMethodHMAC);!ok{ //hmac for our hs256 and prevent token spoofing
		return nil,err
	}
...
func GetUserIdFromContext(c *gin.Context)(string, error){
	userID,exists:=c.Get("userId");if !exists{
		return "",errors.New("userId does not exist in this context")
	}

	id,ok:=userID.(string);if !ok{
...
func GetRoleFromContext(c *gin.Context)(string, error){
	role,exists:=c.Get("role");if !exists{
		return "",errors.New("role does not exist in this context")
	}

	memberRole,ok:=role.(string);if !ok{
		return "",errors.New("unable to retrieve userId")
	}
